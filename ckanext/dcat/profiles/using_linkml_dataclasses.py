...
example_dataset = {
    ...
    'measurement_technique_iri': 'http://purl.obolibrary.org/obo/CHMO_0000604',
...
def graph_from_dataset(dataset_dict):
    ...
    compound_chem = ChemicalEntity(
            ...
            inchi=InChi(title='assigned InChi',
                        value=dataset_dict.get('inchi')),
            smiles=SMILES(title='assigned SMILES',
                          value=dataset_dict.get('smiles')),
        )
    ...
    if dataset_dict.get('mol_formula'):
        # DCAT-AP+
        compound.has_quantitative_attribute.append(QualitativeAttribute(
            rdf_type=DefinedTerm(
        ...
        # ChemDCAT-AP
        compound_chem.molecular_formula = MolecularFormula(title='assigned IUPAC chemical formula',
                                                           value=dataset_dict.get('mol_formula'))
