...
class DCATNFDi4ChemProfile(EuropeanDCATAPProfile):
    ...
    def graph_from_dataset(self,dataset_dict,dataset_ref):
        ...
        # Instantiate the EvaluatedEntity (sample)
        # TODO: We used a fake ID, as the real one is not yet given in the source data.
        ...
        sample = EvaluatedEntity(
            ...
            # Instantiate the compound of which the sample is composed of
            has_part=[
                Entity(
                    ...
                    rdf_type=DefinedTerm(id="CHEBI:23367", title="molecular entity")
                )]
        )
        ...
        # Instantiate the DataGeneratingActivity
        # --- DataGeneratingActivity ---
        measurement = None
        ...
        nfdi_graph.add(rdf_nfdi_dumper.as_rdf_graph(sample, schemaview=schemaview))
        nfdi_graph.add(rdf_nfdi_dumper.as_rdf_graph(analysis, schemaview=schemaview))
        nfdi_graph.add(rdf_nfdi_dumper.as_rdf_graph(spectrum, schemaview=schemaview))
        if measurement is not None:
            nfdi_graph.add(rdf_nfdi_dumper.as_rdf_graph(measurement, schemaview=schemaview))
